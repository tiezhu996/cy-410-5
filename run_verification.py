...
log("\n[环境检查]")
log("------------------------------------------------------------")
run("python3 -c \"import xlrd; print('xlrd版本:', xlrd.__version__, '(需要>=1.2,<2.0)')\"")
run("python3 -c \"import openpyxl; print('openpyxl 版本:', openpyxl.__version__)\"")
run("python3 -c \"import pandas; print('pandas 版本:', pandas.__version__)\"")
...
assert ok, "文件格式不对"
r = run(f"python3 main.py import --file {XLS} --db /tmp/test_heritage.db", cwd=CWD)
...
log("\n[验证2: 查询确认3条数据均入库]")
log("------------------------------------------------------------")
r = run("python3 main.py query --db /tmp/test_heritage.db --limit 10 --format table", cwd=CWD)
...
log("------------------------------------------------------------")
r = run("python3 main.py clean --db /tmp/test_heritage.db", cwd=CWD)
...
log("------------------------------------------------------------")
r = run(["python3", "main.py", "query", "--db", "/tmp/test_heritage.db", "--where", "declare_year>=2006", "--format", "json"], cwd=CWD)
...
log("------------------------------------------------------------")
r = run(["python3", "main.py", "query", "--db", "/tmp/test_heritage.db", "--where", "declare_year>=2008", "--format", "json"], cwd=CWD)
data = json.loads(r.stdout)
...
r = run("python3 main.py export --db /tmp/test_heritage.db --format sqlite --output /tmp/export_target.db", cwd=CWD)
...
r = run("python3 main.py import --file /tmp/sample.xlsx --db /tmp/test_xlsx.db", cwd=CWD)
